...
serve(async (req) => {
  ...
  console.log('[CustomVoiceWidget] Loading enhanced voice widget...');
  ...
  function getConfiguration() {
    ...
    for (let i = 0; i < scripts.length; i++) {
      ...
      if (script.src && script.src.includes('voice-assistant-embed.js')) {
        config = {
          ...
          publicKey: script.getAttribute('data-public-key') || script.dataset.publicKey
        };
        break;
      }
    }
    
    console.log('[CustomVoiceWidget] Config:', config);
    return config;
  }
  ...
  class CustomVoiceWidget {
    ...
    createWidget() {
      ...
      // Create widget card (initially hidden)
    ...
    async loadVapiSDK() {
      ...
      script.onerror = () => this.updateState('error', 'Failed to load SDK');
    ...
    toggleMute() {
      ...
      if (this.vapi && this.vapi.isMuted !== undefined) {
  ...
  if (config.vapiAssistantId && config.publicKey) {
    new CustomVoiceWidget(config);
  } else {
    console.error('[CustomVoiceWidget] Missing required configuration');
  }
})();`;
