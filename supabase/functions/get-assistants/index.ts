...
serve(async (req) => {
  ...
  try {
    ...
    const { data: assistants, error } = await supabase
      ...
      .select(`
        *,
        embed_mappings!vapi_assistant_id(embed_id)
      `)
      .eq('user_id', userId)
    ...
    // Flatten embed_id into assistant objects
    const flattenedAssistants = assistants?.map(assistant => ({
      ...assistant,
    ...
    return new Response(
      JSON.stringify({
        success: true,
        assistants: flattenedAssistants
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
